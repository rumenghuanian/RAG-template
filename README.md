...
> 把自己量具的 **6 次故障**和它们造成的假结论全部抓出来、记下来。
...
判分器写错一次，结论就全废一次。这里 6 次故障都差点变成「结论」，每次都是靠**对照实验**抓出来的：
...
> ⚠️ **生成层数字目前是「week16 之前」的那一套**：下面这些答案是 99 篇语料时生成的，
> 对新增的 6 条题（`expect` 指向 week16）必然「未命中、未引用」。
> 所以旧报告已归档为 `eval/last_gen_report.pre_week16.json`，**没有进新基线**。
> 生成层要重跑一次生成才有有效数字（约 29.5 分钟 + 真实 API 费用）。

| 臂 | 上下文命中 | 引对 | 纯弃答 | **引对率** |
|---|---|---|---|---|
| `single` 单轮 | 86.8% | 92.1% | 0.0% | **86.8%** |
| `agent` | 94.7% | 92.1% | 2.6% | **92.1%** |

按题型拆（引对率，single / agent）：

| 题型 | `single` | `agent` |
|---|---|---|
| `easy` | 100.0% | 100.0% |
| `paraphrase` | 66.7% | **91.7%** |
| `rare_token` | 100.0% | 90.9% |
| `multi` | 80.0% | 80.0% |

闸门题（正确行为是声明「笔记里没有」）：`single` **95.0%**、`agent` **75.0%** 声明了缺口。

### 答案正确性：用「要点覆盖率」当代理，并验证它真的有区分力

LLM 裁判试了 5 版，前 4 版都不可信（见下）；第 5 版修掉参照物截断后，**小样本上验证过
有区分力**（真 6/6 vs 打乱 0/6，注入假细节 3/3 抓出），但**全量忠实度仍未测量**。
所以日常口径换成一个**完全确定、可复算**的办法：
...
**这两个对照证明的是「裁判不是橡皮图章」，不证明「它算得准」。** 而且这一轮 n=6、全是
最可能忠实的 `easy` 题，属于**下限测试**。真正难判的是「措辞合理、来源是上下文里另一篇
文章」的断言 —— 那需要人工复核，不能靠这个数交差。

### 全量忠实度：**仍未测量**（别引用任何数字）

现有全量报告（`eval/last_gen_report.json`）是**加 `context_texts` 之前**生成的，
里面没有「模型实际读了什么」这个字段，`--judge-only` 只能退回用文章全文当参照（脚本会大声警告）。

要拿到全量忠实度，必须先重跑一次生成（78 题 × 2 臂，约 29.5 分钟 + 真实 API 费用），
裁判还要再花 76 次调用（每次参照物约 1.5 万 token）。**在此之前，任何「忠实度 X%」都是没有依据的。**
...
装了完整依赖时全部用例都会跑（83 项）；只装了部分依赖时，需要 faiss 的用例会自动 skip。
...
- 评测集只有 43 条，其中闸门类仅 5 条 —— 「重排分数能分开真·语料外」这个结论基于 **n=3**，
  且这个间隙会随重排池深变化（整池时 +0.142，截到 50 时缩到 +0.069）。**要定阈值必须先扩样。**
...
- basic_rag **v1.4.0**
  ...
  - **已知未做**：全量忠实度仍未测（且换语料后必须重新生成答案）；生成层基线待补
- basic_rag **v1.3.0**
