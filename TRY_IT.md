...
| **3** | 事实正确性 | 3a/3b/3c **全部完成**：全量忠实度 **97.7%**（43/44）+ 人工基准一致率 **97.4%**（37/38） | 已花 |
...
随后按 105 篇语料重跑生成了全量数字（**97.7%**），并配了人工基准。根因之一：
...
| ⑦ | 校准探针拿**修 ⑥ 之前**生成的报告当种子（记的上下文 24,567 字 > 预算，是未截断全文） | 裁判读到模型根本没看到的那段 → ① 判 `False`，输出**「裁判校准不过关」**。是**种子数据**坏了，不是裁判坏了 |
...
### 3c：全量忠实度 —— **已测出 97.7%（43/44）** ✅

```powershell
# ① 重新生成一次（旧全量报告是加 context_texts 之前生成的）—— 830 秒 + 真实费用
.\.venv\Scripts\python.exe eval\run_gen_eval.py
# ② 拿真实上下文裁判全量：44 条可答 = 44 次裁判调用
.\.venv\Scripts\python.exe eval\run_gen_eval.py --judge-only
# ③ 人工基准（我逐条标 44 条，然后算一致率）
.\.venv\Scripts\python.exe eval\build_human_review.py      # 生成 human_review.html
.\.venv\Scripts\python.exe eval\score_human_review.py      # 标完导出 human_labels.json 再跑
```

**实测**（`eval/last_gen_report.json`，`single` 臂，105 篇语料）：

| 口径 | 数值 | 说明 |
|---|---|---|
| 裁判忠实度 | **97.7%**（43/44） | 0 次解析失败 |
| 唯一的不忠实 | 1 条（`rare_token`：Docker Compose 部署微服务） | **覆盖度误述**（错说「笔记没讲到高可用/扩缩容」），不是编造细节 |
| 人工一致率 | **97.4%**（37/38） | 6 条我标「拿不准」不计入；偏严 1 条、**偏宽 0 条** |

**这三个数怎么读**（写简历/README 时别只引第一个）：

- 97.7% 与 97.4% 高度一致，说明**裁判与我这个人的判断基本合拍**，且那一条分歧是口径差异。
- 但**人工样本里一条「不忠实」都没有** → 「偏宽 0 条」是空的，**证明不了裁判抓得住编造**。
  抓编造只有注入探针那条路（把已知忠实的答案人为弄坏）。
- 6 条「拿不准」全是关于**取舍与结构**（「没有相关信息就别硬扯」等），**没有一条是编造** ——
  忠实性这个口径盖不住它们，这是**口径的边界**，不是裁判的错。
...
- ~~**全量忠实度仍未测量**~~ → **已测：97.7%（43/44）**，并配了 44 条人工基准（一致率 97.4%）。
  仍缺的是**偏宽方向**的证据：人工样本里没有一条「不忠实」，只能靠注入探针。
- **`agent` 臂的生成层没按 105 篇语料重跑**：README 里 92.1% 那组是 99 篇语料时的数字
  （已归档 `last_gen_report.pre_week16.json`），别和单轮的 88.6% 混着引用。
